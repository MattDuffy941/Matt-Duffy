...
- **Sound kits** — the "Sound" selector switches between the built-in
  procedural synth (zero-latency default) and sampled kits. Bundled, both
  **CC0 1.0** (public domain — free for any use, no attribution required):
  - **Acoustic** — kick, snare, hi-hats (closed/open/foot), crash and toms
    from the
    [Versilian Community Sample Library](https://github.com/sgossner/VCSL),
    downmixed to mono (~2.5 MB).
  - **TR-808** — from
    [tidalcycles/sounds-tr808-fischer](https://github.com/tidalcycles/sounds-tr808-fischer)
    (~1 MB).

  Sample kits load lazily and fall back to the synth for any voice they don't
